...
if [[ "$RELEASE_MODE" == "1" ]]; then
	npm whoami >/dev/null || {
		echo "WARNING: Not logged in to npm; git push/tag will still run but publish is disabled." >&2
	}
...
if [[ "$RELEASE_MODE" == "1" ]]; then
	...
	echo "=== Tag v$VERSION pushed (npm publish disabled; run manually when ready) ==="
else
