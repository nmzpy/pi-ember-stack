...
import { grokSupportsReasoningEffort, isGrokCliProxyModel } from "./models.js";
import { textFromResponsesContent } from "./text.js";
	...
	if (obj.type === "image_url") {
		const imageUrl =
			typeof obj.image_url === "object" && obj.image_url
				? (obj.image_url as JsonObject).url
				: obj.image_url;
		const detail =
			typeof obj.image_url === "object" && obj.image_url
				? (obj.image_url as JsonObject).detail
				: obj.detail;
...
function isResponsesInputImagePart(value: unknown): value is JsonObject {
	return !!value && typeof value === "object" && (value as JsonObject).type === "input_image";
}
...
function normalizeXaiResponsesInput(input: unknown[], model: Model<Api>): unknown[] {
	const normalizedInput = input.map(normalizeResponsesImageParts) as JsonObject[];
	const rewritten: unknown[] = [];
	const modelInputs = Array.isArray((model as unknown as JsonObject).input)
		? ((model as unknown as JsonObject).input as unknown[])
		: [];
	...
	for (const item of normalizedInput) {
		...
		if (supportsImages && imageParts.length > 0) {
			const label = `The previous tool result${item.call_id ? ` (${item.call_id as string})` : ""} included ${imageParts.length} image${imageParts.length === 1 ? "" : "s"}. Use the attached image${imageParts.length === 1 ? "" : "s"} as the visual output from that tool.`;
			rewritten.push({
				role: "user",
...
export function rewriteXaiResponsesPayload(
	...
	const body: JsonObject = { ...(payload as JsonObject) };
	const modelId = String(body.model || model.id);
	const usesGrokCliProxy = isGrokCliProxyModel(modelId);

	if (Array.isArray(body.input)) {
		let input = normalizeXaiResponsesInput([...body.input], model) as JsonObject[];
	...
	if (body.reasoning && typeof body.reasoning === "object") {
		const effort = (body.reasoning as JsonObject).effort;
	...
	if (usesGrokCliProxy && Array.isArray(body.include)) {
		...
		if ((body.include as unknown[]).length === 0) delete body.include;
	}
