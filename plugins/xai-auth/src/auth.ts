...
import { ensureFreshXaiCredentials } from "./oauth.js";
	...
	try {
		...
		if (topLevelAccess) {
			return {
				...
				expires:
					parseExpiry(data.expires_at || data.expires) || Date.now() + 30 * 24 * 60 * 60 * 1000,
				tokenEndpoint: `${XAI_OAUTH_ISSUER}/oauth2/token`,
...
export async function resolveXaiAuthToken(ctx: ExtensionContext): Promise<string | null> {
