...
test("index.ts imports buildSearchErrorPlan and wires it into the web_search error path", () => {
	...
	// Web-tool rows use the compact-tool bullet SSOT and return the compact text
	// directly — no shell, so nothing pads a column in front of the bullet.
	// Success/error backgrounds must not return on a future refactor.
	assert.match(indexSrc, /statusBulletColor/);
	assert.match(indexSrc, /return compactText;/);
	assert.doesNotMatch(indexSrc, /new Box\(/);
