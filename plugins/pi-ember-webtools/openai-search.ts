...
const AUTH_MODEL_CANDIDATES: ReadonlyArray<{
	provider: "openai-codex" | "openai";
	models: ReadonlyArray<string>;
}> = [
	{
		provider: "openai-codex",
		models: ["gpt-5.4", "gpt-5.3-codex", "gpt-5.3-codex-spark", "gpt-5.2", "gpt-5.2-codex"],
	},
...
export async function resolveOpenAIAuth(ctx?: ExtensionContext): Promise<OpenAIAuth | undefined> {
	if (ctx) {
		...
		for (const candidate of AUTH_MODEL_CANDIDATES) {
			for (const modelId of candidate.models) {
				// biome-ignore lint/suspicious/noExplicitAny: model IDs are validated at runtime by getModel returning undefined
				const model = getModel(candidate.provider, modelId as any);
				if (!model) continue;
				try {
	...
	const apiKey =
		normalizeApiKey(process.env.OPENAI_API_KEY) ?? normalizeApiKey(loadConfig().openaiApiKey);
