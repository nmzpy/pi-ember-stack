...
**Web search, content extraction, and video understanding for Pi agent. Zero-config Exa search, Brave, Parallel, Tavily, optional browser-cookie Gemini Web, or bring your own API keys.**
...
**Zero Config** — Works out of the box with Exa MCP (no API key needed). Add API keys for Brave, Parallel, Tavily, Exa, Perplexity, or Gemini API for more control, or opt into browser-cookie access for Gemini Web.

**Video Understanding** — Point it at a YouTube video or local screen recording and ask questions about what's on screen. Full transcripts, visual descriptions, and frame extraction at exact timestamps.

**Smart Fallbacks** — Every capability has a fallback chain. Search tries Exa, then Brave, Parallel, Tavily, Perplexity, Gemini API, and Gemini Web when browser cookies are enabled. YouTube tries Gemini Web when enabled, then API, then Perplexity. Blocked pages retry through Jina Reader, Parallel, and Gemini extraction. Something always works.
...
In `auto` mode (default), `web_search` tries Exa (direct API if keyed, MCP if not), Brave, Parallel, Tavily, Perplexity, Gemini API, then Gemini Web when browser-cookie access is enabled.
...
Search the web via Exa, Brave, Parallel, Tavily, Perplexity AI, or Gemini. Returns a synthesized answer with source citations.
...
| `provider` | `auto` (default), `exa`, `brave`, `parallel`, `tavily`, `perplexity`, or `gemini` |
| `includeContent` | Fetch full page content from sources in background |
| `workflow` | `none` (skip curator, default), `summary-review` (open curator and auto-generate a summary draft), or `auto-summary` (generate a summary without opening the curator) |
...
web_search(query)
web_search(query)
  → Exa (direct API with key, MCP without) → Brave → Parallel → Tavily → Perplexity → Gemini API → Gemini Web (if browser cookies enabled)
...
| `search.ts` | Search routing across Exa, Brave, Parallel, Tavily, Perplexity, Gemini API, Gemini Web |
