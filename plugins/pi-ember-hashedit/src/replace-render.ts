...
import type { ReplaceDetails, ReqParams } from "./replace.ts";
...
function isStringArray(value: unknown): value is string[] {
	return (
		Array.isArray(value) && value.every((line: unknown): line is string => typeof line === "string")
	);
}
...
export function getPreviewInput(args: unknown): ReqParams | null {
	...
	const { path, remove_from, remove_to, replacement_lines } = normalized;
	if (
		typeof path !== "string" ||
		typeof remove_from !== "string" ||
		typeof remove_to !== "string" ||
		!isStringArray(replacement_lines)
	) {
		return null;
	}
