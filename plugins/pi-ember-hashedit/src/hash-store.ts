...
import { contentChecksum, initHasher } from "./hashline/hasher.ts";
import { hashStoreDir, hashStorePath, legacyHashStorePath } from "./paths.ts";
import { errCode, isRec, splitLines } from "./utils.ts";
...
function openDbWithBusyRetry(storePath: string): { db: SqliteDatabase; stmts: Prepared } {
	return withBusyRetry(() => openDb(storePath));
}

let cachedDb: { path: string; db: SqliteDatabase; stmts: Prepared } | null = null;
...
function openDb(storePath: string): { db: SqliteDatabase; stmts: Prepared } {
...
function buildStore(db: SqliteDatabase): { db: SqliteDatabase; stmts: Prepared } {
...
function shutdownDb(db: SqliteDatabase): void {
	try {
...
async function openStore(storePath: string): Promise<HashStore> {
	...
	let opened: { db: SqliteDatabase; stmts: Prepared };
	try {
...
async function migrateLegacy(db: SqliteDatabase): Promise<void> {
