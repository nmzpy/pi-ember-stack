...
import type { LFile } from "./file-kind.ts";
...
export function valKind(
	...
	if (file.kind === "binary") {
		throw new Error(
			`[E_NOT_TEXT] Path is a binary file: ${path} (${file.description}). Hashline edit only supports text files.`,
		);
	}
	if (file.kind === "image") {
		throw new Error(
			`[E_NOT_TEXT] Path is an image file: ${path}. Hashline edit only supports text files.`,
		);
	}
