import { getSnapshot, type HashStore, loadHashStore, upsertSnapshot } from "../hash-store.ts";
import { requireArrayItem, splitLines } from "../utils.ts";
import { ALPH, ALPH_RE, HASH_CLASS, HASH_LEN } from "./alphabet.ts";
...
function idxToHash(idx: number): string {
	...
	for (let j = 0; j < HASH_LEN; j++) {
		const char = ALPH[idx % ALPH.length];
		if (char === undefined) {
			throw new Error(`[E_INTERNAL] Missing hash alphabet character at index ${idx}.`);
		}
		out = char + out;
...
// Near-miss prefix: a 2-4 char alphanumeric run followed by the hashline separator.
// Catches pasted read rows whose hash length is off by one (e.g. a 4-char `P9n2│`),
// so a literal `│` never leaks into the file when the exact {3} regex misses.
export const HL_FUZZY_PREFIX_RE = new RegExp(`^\\s*([A-Za-z0-9]{2,4})│`);
...
export function _lineHashesPure(content: string): string[] {
	...
	for (let i = 0; i < lines.length; i++) {
		const c = canon(requireArrayItem(lines, i));
...
function nearestNew(candidates: number[], target: number): number {
	...
	while (lo < hi) {
		const mid = (lo + hi) >>> 1;
		if (requireArrayItem(candidates, mid) < target) lo = mid + 1;
		else hi = mid;
	}
	const left = lo - 1;
	const right = lo;
	if (
		...
		(right >= candidates.length ||
			target - requireArrayItem(candidates, left) <= requireArrayItem(candidates, right) - target)
	) {
		return left;
	}
...
function mapStableHashes(
	...
	for (let i = 0; i < oldHashes.length; i++) {
		const hash = requireArrayItem(oldHashes, i);
	...
	for (let i = 0; i < oldLines.length; i++) {
		const entry = { index: i, hash: requireArrayItem(oldHashes, i) };
	...
	for (let i = 0; i < newLines.length; i++) {
		const key = canon(requireArrayItem(newLines, i));
	...
	for (const entry of survivors) {
		const candidates = newByContent.get(canon(requireArrayItem(oldLines, entry.index)));
		if (!candidates || candidates.length === 0) continue;
		const target = entry.index > spanEnd ? entry.index + shiftAfterSpan : entry.index;
		const pos = nearestNew(candidates, target);
		if (pos < 0) continue;
		const newIdx = requireArrayItem(candidates.splice(pos, 1), 0);
	...
	for (const entry of removedEntries) {
		const key = requireArrayItem(oldLines, entry.index);
	...
	for (let i = 0; i < newLines.length; i++) {
		...
		const queue = removedByContent.get(requireArrayItem(newLines, i));
		if (!queue || queue.pos >= queue.hashes.length) continue;
		newHashes[i] = requireArrayItem(queue.hashes, queue.pos);
		queue.pos += 1;
	}

	for (let i = 0; i < newLines.length; i++) {
		if (newHashes[i]) continue;
		const c = canon(requireArrayItem(newLines, i));
		const baseIdx = (xxh32(c) >>> 14) % HASH_SPACE;
