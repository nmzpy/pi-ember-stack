import { abortIf, requireArrayItem, splitLines } from "../utils.ts";
...
export function buildIdx(content: string): LIdx {
	...
	for (let index = 0; index < fileLines.length; index++) {
		...
		offset += requireArrayItem(fileLines, index).length;
...
function resToSpan(edit: RHEdit, content: string, lineIndex: LIdx): RESpan | NoopSpan {
	...
	if (edit.content_lines.length > 0) {
		return {
			kind: "replace",
			start: requireArrayItem(lineStarts, startLine - 1),
			end:
				requireArrayItem(lineStarts, endLine - 1) + requireArrayItem(fileLines, endLine - 1).length,
	...
	if (endLine < fileLines.length) {
		return {
			kind: "replace",
			start: requireArrayItem(lineStarts, startLine - 1),
			end: requireArrayItem(lineStarts, endLine),
			replacement: "",
		};
	}
	...
	return {
		...
		start:
			prevLine !== undefined && prevLine.length === 0
				? requireArrayItem(lineStarts, startLine - 1)
				: Math.max(0, requireArrayItem(lineStarts, startLine - 1) - 1),
		end: content.length,
		replacement: "",
	};
}
...
export function applyEdit(
	...
	if (mismatches.length || !initialResolved) {
		const feedback = fmtMismatchWithHashes(mismatches, lineIndex.fileLines, fileHashes, filePath);
