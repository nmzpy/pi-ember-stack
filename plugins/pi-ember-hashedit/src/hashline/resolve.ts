...
function resAnchorFromMap(ref: Anchor, hashIndex: Map<string, number[]>): RAnchor | HMismatch {
	...
	if (hashMatches.length === 1) {
		return {
			line: requireArrayItem(hashMatches, 0),
			hash: ref.hash,
			hashMatched: true,
		};
	}
	...
	if (notFound.length > 0) {
		...
		for (const m of notFound) {
			...
			for (let ln = from; ln <= to; ln++) {
				hashes.push(requireArrayItem(fileHashes, ln - 1));
		...
		for (const m of ambiguous) {
			...
			const more =
				(m.candidates?.length ?? 0) > sample.length
					...
					: "";
			for (const line of sample) hashes.push(requireArrayItem(fileHashes, line - 1));
			const lines = sample
				.map((line) => {
...
function assertItem(edit: Record<string, unknown>): void {
	rejectUnknownFields(
		...
		ITEM_KS,
		"Edit",
		"The edit takes only { replacement_lines, remove_from, remove_to }.",
	);
	...
	if (!("replacement_lines" in edit)) {
		throw new Error(
			`[E_BAD_SHAPE] The edit requires a "replacement_lines" field. Provide the replacement lines as an array of strings (use [] to delete).`,
		);
	}
	if (
		!Array.isArray(edit.replacement_lines) ||
		edit.replacement_lines.some((line) => typeof line !== "string")
	) {
...
export function resEdit(edit: HTEdit, warnings?: string[]): HEdit {
	...
	const bounds = [edit.remove_from, edit.remove_to].map((ref) => {
		...
		if (match) {
			...
			return requireArrayItem(match, 2);
		}
		return ref;
	}) as [string, string];
	return {
...
export function stripBarePrefixes(edit: HEdit, fileHashes: string[], warnings: string[]): HEdit {
	...
	// `exact` = a real 3-char hash prefix; `fuzzy` = a 2-4 char run that only matches when exact missed.
	type Strip = { lineIndex: number; kind: "exact" | "fuzzy"; matched: boolean };
	const stripped: Strip[] = [];
	const contentLines = edit.content_lines.map((line, lineIndex) => {
		const exact = line.match(HL_BARE_PREFIX_RE);
		if (exact) {
			const hash = requireArrayItem(exact, 1);
			stripped.push({ lineIndex, kind: "exact", matched: fileHashSet.has(hash) });
			return line.slice(exact[0].length);
		}
		const fuzzy = line.match(HL_FUZZY_PREFIX_RE);
		if (fuzzy) {
			const run = requireArrayItem(fuzzy, 1);
			stripped.push({ lineIndex, kind: "fuzzy", matched: fileHashSet.has(run) });
			return line.slice(fuzzy[0].length);
		}
		return line;
	});
	...
	const loc = (s: Strip) =>
		`replacement_lines line ${s.lineIndex + 1}${s.kind === "fuzzy" ? " (near-miss prefix)" : ""}`;
	const locations = risky.map(loc).join(", ");
	const fuzzyCount = risky.filter((s) => s.kind === "fuzzy").length;
	const matchedCount = risky.filter((s) => s.matched).length;
	const evidence =
		matchedCount === 0
			? "none of the stripped hashes match current file lines"
			: `${matchedCount} of ${risky.length} stripped hash(es) match current file lines`;
	const guidance =
		matchedCount === 0
			? " Verify that these lines were pasted from read output; literal content starting with 'HASH│' would be altered by this strip. replacement_lines must never contain a HASH│ prefix — pass bare content only."
			: "";
	const fuzzyNote =
		fuzzyCount > 0
			? ` Stripped ${fuzzyCount} near-miss prefix(es) (2-4 char run + │) to prevent a literal '│' leaking into the file.`
			: "";
	warnings.push(
		`[E_BARE_HASH_PREFIX] Autocorrected: stripped "HASH│" prefix copied from read output in ${locations} (${evidence}).${guidance}${fuzzyNote}`,
	);
...
export function swapReversedRanges(edit: HEdit, fileHashes: string[], warnings: string[]): HEdit {
	...
	for (let i = 0; i < fileHashes.length; i++) {
		lineByHash.set(requireArrayItem(fileHashes, i), i + 1);
	}
...
function firstNewAfterDups(
	...
	while (
		runLen < maxK &&
		canon(requireArrayItem(contentLines, firstNew.index + runLen)) ===
			requireArrayItem(canonLines, endLine + runLen)
	) {
		runLen++;
	}
...
function lastNewBeforeDups(
	...
	while (
		runLen < maxK &&
		canon(requireArrayItem(contentLines, lastNew.index - runLen)) ===
			requireArrayItem(canonLines, startLine - 2 - runLen)
	) {
		runLen++;
	}
	if (runLen === 0) return [];
...
export function findNewEdge(
	...
	for (let i = start; i >= 0 && i < contentLines.length; i += step) {
		const line = requireArrayItem(contentLines, i);
...
export function valEdit(
	...
	for (let i = 0; i < fileHashes.length; i++) {
		const h = requireArrayItem(fileHashes, i);
	...
	if (!startResolved || !endResolved) {
		if (!startResolved && endResolved) {
			let startMismatch: HMismatch | undefined;
			for (let index = mismatches.length - 1; index >= 0; index--) {
				const mismatch: HMismatch | undefined = mismatches[index];
				if (mismatch?.ref === edit.hash_bounds[0]) {
					startMismatch = mismatch;
					break;
				}
			}
			if (startMismatch?.kind === "not_found") startMismatch.context = endResolved;
		} else if (startResolved && !endResolved) {
			let endMismatch: HMismatch | undefined;
			for (let index = mismatches.length - 1; index >= 0; index--) {
				const mismatch: HMismatch | undefined = mismatches[index];
				if (mismatch?.ref === edit.hash_bounds[1]) {
					endMismatch = mismatch;
					break;
				}
			}
			if (endMismatch?.kind === "not_found") endMismatch.context = startResolved;
		}
...
export function assertRangeServed(
	...
	for (let line = startLine; line <= endLine; line++) {
		if (!served.has(requireArrayItem(fileHashes, line - 1))) mismatchLines.push(line);
	}
