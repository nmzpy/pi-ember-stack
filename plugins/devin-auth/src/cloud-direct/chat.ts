...
function getOrAllocateSessionIds(
	apiKey: string,
	host: string,
...
function encodeChatMessagePrompt(
	...
	const imageParts = content.filter(
		(p): p is { type: "image"; mimeType: string; base64Data: string; caption?: string } =>
			p.type === "image",
	);
...
export async function* streamChatEvents(req: CloudChatRequest): AsyncGenerator<CloudChatEvent> {
	...
	const ttfbTimer = setTimeout(
		() =>
			ttfbController.abort(
				new Error(`cloud-direct: time-to-first-byte timeout (${CLOUD_STREAM_TTFB_MS}ms)`),
			),
	...
	if (!resp.ok) {
		...
		throw new CloudChatError(
			`GetChatMessage HTTP ${resp.status}: ${text.slice(0, 300)}`,
			undefined,
		);
	}
	if (!resp.body) {
	...
	try {
			...
			return new Promise((resolve, reject) => {
				...
				idleController.signal.addEventListener(
					...
					() => {
						...
						try {
							void reader
								.cancel(idleController.signal.reason ?? new Error("idle abort"))
								.catch(() => {
									/* swallowed; outer race already rejected */
								});
						} catch {
							/* */
						}
		...
		try {
			void reader.cancel().catch(() => {
				/* swallowed; stream already closed or aborted */
			});
		} catch {
			/* */
		}
	...
	if (trailerError) {
		...
		if (isOpaquePermissionDenial) {
			const enriched = `Cognition denied this request for model "${req.modelUid}" with the opaque "an internal error occurred" message. This almost always means the model is not enabled for your account/tier — see https://codeium.com/account. (cloud trace ID: ${trailerError.traceId ?? "n/a"}; raw message: ${trailerError.message})`;
